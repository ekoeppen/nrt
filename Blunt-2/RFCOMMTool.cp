			...
			case E_CONNECTION_COMPLETE: {
				...
				if (e->fResult == noErr) {
					...
					if (e->fHandler->fHandlerType == H_RFCOMM) {
						...
						// FIX: Set official state.
						fState = 2; // kStateConnected
						
						TCommToolGetEventReply reply;
						reply.fEventCode = 1;
						reply.fEventData = kSerialEventDCDAssertedMask;
						reply.fServiceId = 'rfcm';

						// FIX: Check return value to prevent event drop
						NewtonErr err = PostCommEvent (reply, noErr);
						if (err == kCommErrNoGetCommEvent) {
							// Deliver later during poll
							fGetEventReply = reply;
						}
					}
				} else {
			...
			case E_DATA: {
				...
				if (e->fResult == noErr) {
					...
					if (e->fLength > 0 && e->fData != NULL) {
						...
						// FIX: Bounds check to prevent heap corruption
						if (e->fLength > n) {
							Size incoming = e->fLength - n;
							if (fSavedDataAmount + incoming > MAX_SAVE) {
								incoming = MAX_SAVE - fSavedDataAmount;
							}
							if (incoming > 0) {
								memcpy (fSavedData + fSavedDataAmount, e->fData + n, incoming);
								fSavedDataAmount += incoming;
							}
						}
